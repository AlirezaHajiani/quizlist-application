...
function GameMain(props) {
  ...
  function checkAnswer1()
  {
    if(answer.length>0)
    {
      let checked = findAnswer(answer,
                   match.Match.questions[match.Match.round].answer1,
                   match.Match.questions[match.Match.round].answer2,
                   match.Match.questions[match.Match.round].answer3);
      let ind = userNumber==1?findIndexInAnswers1(checked[0],match.Match.answer1[match.Match.round]):
                              findIndexInAnswers1(checked[0],match.Match.answer2[match.Match.round]);
      if(ind<0)
      {
        userNumber==1?match.Match.answer1[match.Match.round].push(checked):match.Match.answer2[match.Match.round].push(checked);
  ...
  function getRandomInt(min, max) {
   ...
   function randomAnswer(){
      let filter1=answersFilter(userNumber==1?match.Match.answer1[match.Match.round]:match.Match.answer2[match.Match.round],match.Match.questions[match.Match.round].answer1);
      let filter2=answersFilter(userNumber==1?match.Match.answer1[match.Match.round]:match.Match.answer2[match.Match.round],match.Match.questions[match.Match.round].answer2);
      let filter3=answersFilter(userNumber==1?match.Match.answer1[match.Match.round]:match.Match.answer2[match.Match.round],match.Match.questions[match.Match.round].answer3);
      ...
      if(filter1.length==0 && filter2.length==0 && filter3.length==0)
      {
        ...
        if(randType==1)
        {
          ...
          if(answer !== undefined)
          {
            userNumber==1?match.Match.answer1[match.Match.round].push([answer,1]):match.Match.answer2[match.Match.round].push([answer,1]);
            userdata.User.coins-=config.JOKER_COIN;
            userdata.User.coinsAdd-=config.JOKER_COIN;
        ...
        else if(randType==2)
        {
          ...
          if(answer !== undefined){
            userNumber==1?match.Match.answer1[match.Match.round].push([answer,2]):match.Match.answer2[match.Match.round].push([answer,2]);
            userdata.User.coins-=config.JOKER_COIN;
            userdata.User.coinsAdd-=config.JOKER_COIN;
            dispatch(userData(userdata.User));
            dispatch(matchData(match.Match));
        ...
        else if (randType==3)
        {
          ...
          if(answer !== undefined)
          {
          userNumber==1?match.Match.answer1[match.Match.round].push([answer,3]):match.Match.answer2[match.Match.round].push([answer,3]);
          userdata.User.coins-=config.JOKER_COIN;
          userdata.User.coinsAdd-=config.JOKER_COIN;
          dispatch(userData(userdata.User));
          dispatch(matchData(match.Match));
  ...
  useEffect(() => {
    ...
    setTutorial(0);
    setTimeValue((config.MAX_TIME-15)/config.MAX_TIME);
    // bgTimer=BackgroundTimer.runBackgroundTimer(() => {
    //   savedCallback.current();
    // },
    // 1000);
    //
    // dispatch(soundPlayMusic("timermetronome"));

    return () => {
      BackgroundTimer.stopBackgroundTimer();
      BackgroundTimer.clearInterval(bgTimer);
      dispatch(soundStopMusic());
    }
  },[])
  ...
  {
    return(
      <View style={{flex:1, position: 'absolute',top:0,left:0,bottom: 0,right: 0,backgroundColor: COLORS.overlay}}>
        <View style={{height: 40, flexDirection: 'row'}}>

          <View style={{width:40,justifyContent: 'flex-end',flexDirection: 'row'}}>
              {tutorial>0?<Text style={[styles.questiontext,{fontSize: 25,marginRight: 5,marginTop: 3,color: COLORS.text}]}>{(config.MAX_TIME*timeValue).toFixed()}</Text>:<></>}
          </View>

          <View style={{flex:1,justifyContent: 'flex-end',alignItems: 'center',flexDirection: 'row'}}>
              {tutorial>0?<View style={{flex:timeValue ,elevation: 8, backgroundColor: `rgb(255,${timeValue*150},${timeValue*150})`,height: 25,borderRadius: 8}}/>:<></>}
          </View>
        ...
        <View style={{flex:1}}>
          ...
          <View style={{...styles.answers,elevation: 0,backgroundColor: null}}>
            <View style={{...styles.question,backgroundColor: null}}>
            ...
            {tutorial==0?
            <View style={{backgroundColor: "#FFFFFF", height: 160,width: 200,borderRadius: 10,padding: 10,alignSelf:'center'}}>
              <Text style={{...styles.questiontext,textAlign: 'center',lineHeight: 30}}>
                قبل از اینکه وقتت تموم بشه جواب های این سوال رو وارد کن
              </Text>
              <AnimButton
                color={COLORS.green}
                backColor={COLORS.greenShadow}
                style={{height: 40,width: 100,alignSelf: 'center',marginLeft: 5,marginRight: 5}}
                onPress={() =>{
                                setTutorial(1);
                              }}>
                  <Text style={[styles.questiontext,{marginTop: 1, marginLeft: 5,marginRight: 5,color: COLORS.text,textAlign: 'center'}]}>خب</Text>
              </AnimButton>
            </View>:<></>}

            {tutorial==3?
            <View style={{backgroundColor: "#FFFFFF", height: 160,width: 200,borderRadius: 10,padding: 10,alignSelf:'center',marginTop: 100}}>
              <Text style={{...styles.questiontext,textAlign: 'center',lineHeight: 30}}>
                جواب هایی که وارد می کنی 1، 2 یا 3 امتیاز دارن
              </Text>
              <AnimButton
                color={COLORS.green}
                backColor={COLORS.greenShadow}
                ...
                onPress={() =>{
                  ...
                  <Text style={[styles.questiontext,{marginTop: 1, marginLeft: 5,marginRight: 5,color: COLORS.text,textAlign: 'center'}]}>شروع</Text>
              </AnimButton>
            </View>:<></>}
          </View>
          {tutorial==1?
          <View style={{backgroundColor: "#FFFFFF", marginBottom: -30,height: 130,width: 150,marginRight: 10,borderRadius: 10,padding: 10,alignSelf:'flex-end'}}>
            <Text style={{...styles.questiontext,textAlign: 'center',lineHeight: 30}}>
              کلید زمان 15 ثانیه وقت اضافه بهت میده. امتحانش کن!
            </Text>
          </View>:<></>}

          {tutorial==2?
          <View style={{backgroundColor: "#FFFFFF", marginBottom: -30,height: 130,width: 150,marginRight: 20,borderRadius: 10,padding: 10,alignSelf:'center'}}>
            <Text style={{...styles.questiontext,textAlign: 'center',lineHeight: 30}}>
              کلید جوکر یکی از جواب ها رو باز میکنه
            </Text>
          </View>:<></>}
            ...
            <View style={styles.textInputContainer}>
              ...
              <View style={{flex:1,flexDirection: 'row'}}>
                ...
                {tutorial==2?
                  ...
                  disabled={userdata.User.coins<config.JOKER_COIN && !allAnswered}
                  onPress={() =>{
                                randomAnswer();
                                setTutorial(3);
                                }}>
              ...
              <View style={{flex:1,flexDirection: 'row'}}>
                ...
                {tutorial==1?
                  ...
                  style={{flex:1,marginLeft: 5,marginRight: 5}}
                  disabled={userdata.User.coins<config.TIME_COIN}
                  onPress={() =>{
                                  // time=1;
                                  userdata.User.coins-=config.TIME_COIN;
                                  userdata.User.coinsAdd-=config.TIME_COIN;
                                  dispatch(userData(userdata.User));
        ...
        <View style={{flex:1, backgroundColor: COLORS.bluemed}}>
          <View style={styles.answers}>
            ...
            <View style={{flex:1}}>
              <FlatList
                ...
                data={userNumber==0?[]:(userNumber==1?match.Match.answer1 && match.Match.answer1[match.Match.round] || []:match.Match.answer2 && match.Match.answer2[match.Match.round] || [])}
                //extraData={userNumber==0?[]:(userNumber==1?match.Match.answer1[match.Match.round]:match.Match.answer2[match.Match.round])}
          ...
          <View style={{height: 100}}>
            ...
            <View style={styles.textInputContainer}>
              ...
              <View style={{flex:.5,flexDirection: 'row',alignItems:'center',justifyContent:'flex-end'}}>
                  <View style={{flexDirection: 'row'}}>
                    <Text style={{color: COLORS.text, fontFamily: "Koodak",fontWeight: '600',fontSize: 18,}}>{0||userdata.User.coins}</Text>
